...
export function FaceScanner({ sessionId, onMatch, records }: FaceScannerProps) {
  ...
  const runMatch = async (video: HTMLVideoElement) => {
    try {
      const full = await faceapi.detectSingleFace(video).withFaceLandmarks().withFaceDescriptor();
      ...
      if (bestMatch.distance < 0.45) {
        ...
        setPromptAndState('NO_MATCH', `❌ No match (score: ${bestMatch.distance.toFixed(2)}) — try again`);
      }

      // Auto-reset after delay
      stableFrames.current = 0;
      setTimeout(() => {
        if (scanStateRef.current === 'SUCCESS' || scanStateRef.current === 'NO_MATCH') {
          setPromptAndState('IDLE', 'Show your face to the camera');
        }
      }, RESET_DELAY_MS);
      ...
      <div style={{ position: 'relative', width: '100%', aspectRatio: '4/3', backgroundColor: '#000', borderRadius: '8px', overflow: 'hidden' }}>
        ...
        {isRunning && prompt && (
          <div style={{
            ...
            fontWeight: 600, fontSize: '0.9rem', zIndex: 30,
            boxShadow: '0 2px 10px rgba(0,0,0,0.4)',
            whiteSpace: 'nowrap', transition: 'background-color 0.25s',
          }}>
            {prompt}
          </div>
        )}
        ...
        {!isRunning && (
          <div style={{
            ...
            {!isReady && <span style={{ color: '#94a3b8', fontSize: '0.8rem' }}>Please wait…</span>}
          </div>
        )}
      </div>
